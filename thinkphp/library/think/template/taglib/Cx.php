...
class Cx extends Taglib
{
    ...
    public function tagLoad($tag, $content)
    {
        ...
        foreach ($array as $val) {
            $val = trim(str_replace(PHP_EOL,'',$val));
            $type = strtolower(substr(strrchr($val, '.'), 1));
            switch ($type) {
                case 'js':
                    $parseStr .= '<script type="text/javascript" src="'. $root . $val . '"></script>';
                    break;
                case 'css':
                    $parseStr .= '<link rel="stylesheet" type="text/css" href="' . $root . $val . '" />';
                    break;
                case 'php':
